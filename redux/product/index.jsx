...
export const getProductById = createAsyncThunk("id/getProductById", async (payload, thunkAPI) => {
  try {
    let response;
    response = await request.get(`product_card/${payload}`).then((response) => response.data);
    // toast(<RequestMessage message="Message sent successfully!" />);
    return response;
  } catch (error) {
    console.log("Error", error);
